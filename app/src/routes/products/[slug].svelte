<script context="module">
  ...
  export const load = async ({ fetch, params, url }) => {
    ...
    const seoDescription = data.seo
      ...
      : "Продукция от компании 'Орбита-Строй'";
    const title =
      "Стройматериалы" +
      " || " +
      nameCategory +
      ...
      seoTitle;
    const description =
      nameCategory + " в Нижнем Новгороде и области. " + seoDescription;
  ...
  <div class="p-8">
    ...
    >
      {#each products as { id, name, slug, size, unit, image }}
        <li
          class="col-span-1 flex flex-col text-center bg-white rounded-lg shadow-lg shadow-slate-200/50 divide-y divide-slate-200 border-2 border-slate-100 ring-offset-1 ring-1 ring-slate-100"
        >
          <a sveltekit:prefetch sveltekit:noscroll href="/product/{slug}">
            <div class="flex-1 flex flex-col p-8">
              ...
              {/if}
              <h3
                class="mt-6 text-slate-900 text-sm font-medium h-10 lowercase first-letter:uppercase"
              >
                {name}-{id}
              </h3>
              <dl class="mt-1 flex-grow flex flex-col justify-between">
                ...
                <dd class="mt-4">
                  <span
                    class="px-3 py-1.5 text-slate-100 text-base font-medium bg-indigo-800 rounded-full"
                    >{size[0].price.price}
                    р/{unit}</span
                  >
                </dd>
